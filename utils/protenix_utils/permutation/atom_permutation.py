...
import torch
from .utils import Checker, save_permutation_error
...
# Key idea is to leverage equivalent permutation into Kabsch algorithm
__all__ = [
    'run'
]
...
def collect_residues_with_symmetric_atoms(
    ...
    res_start_end = list(zip(start_positions[:-1].tolist(), start_positions[1:].tolist()))  # [N_res, 2]
    N_res = len(res_start_end)
    assert N_res == len(torch.unique(ref_space_uid))

    position_list: list[Tuple[int, int]] = []
    perm_list: list[torch.Tensor] = []          # many of [N_perm, N_res_atom]
    coord_list: list[torch.Tensor] = []         # many of [N_res_atom]
    coord_mask_list: list[torch.Tensor] = []    # many of [N_res_atom]    
    ...
    for start, end in res_start_end:
        ...
        perm: torch.Tensor = torch.unique(perm, dim=-1) # [N_res_atom, N_perm]
...
class AtomPermutation(object):
    ...
    def global_align_pred_to_true(
        ...
        # COMMENT: if pred's batched, true must be batched(indexed before calling) 

        with torch.cuda.amp.autocast(enabled=False):
            aligned_rmsd, transformed_pred_coord, _, _ = rmsd_globally_aligned(
    ...
    def __call__(
        ...
        # log_dict['per_res_optimized_rmsd'] = (
        #     (torch.stack(residue_optimized_rmsd_list, dim=-1) * is_res_permuted)
        #     .mean()
        #     .item()
        # )
        # log_dict['per_res_original_rmsd'] = (
        #     (torch.stack(residue_original_rmsd_list, dim=-1) * is_res_permuted)
        #     .mean()
        #     .item()
        # )
