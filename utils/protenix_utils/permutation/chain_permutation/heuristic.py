...
def _correct_symmetric_chains_for_one_sample(
    ...
    **kwargs,
    ...
    if permute_label:
        ...
        permute_pred_indices: torch.Tensor
...
class MultiChainPermutation(object):
    ...
    @staticmethod
    def check_pattern_and_create_mapping(mol_id1, mol_id2):
    ...
    @staticmethod
    def _convert_to_per_asym_feature_dict(asym_id, feature_dict):
        out = {}
    ...
    def find_anchor_asym_chain_in_predictions(self):
        """
    ...
    @staticmethod
    def _select_atoms_by_mol_atom_index(input_dict, mol_atom_index):
    ...
    def compute_best_match_heuristic(self):
        ...
        if self.find_gt_anchor_first:
            # Randomly sample a GT asym chain using this entity id
            anchor_gt_asym_id = self.label_token_dict["entity_to_asym"][
                anchor_entity_id
            ].tolist()
            anchor_gt_asym_id = random.choice(anchor_gt_asym_id)
            # The anchor asym is from GT
            anchor_asym_dict = self.label_asym_dict[anchor_gt_asym_id]
            # The candidate anchors to be matched are from Pred
            candidate_anchors = self.pred_token_dict["entity_to_asym"][anchor_entity_id]
        else:
            # The anchor asym is from Pred
            anchor_asym_dict = self.pred_asym_dict[anchor_pred_asym_id]
            # The candidate anchors to be matched are from GT
    ...
    def _calculate_rmsd(self, pred_asym_id, gt_asym_id):
    ...
    def match_pred_asym_to_gt_asym(self, pred_asym_id, gt_asym_ids):
    ...
    @staticmethod
    def build_permuted_indice(pred_dict, label_full_dict, best_match: dict[int, int]):
        """
        Args:
    ...
    @staticmethod
    def aligned_rmsd(
        pred_dict, label_full_dict, indices, reduce: bool = True, eps: float = 1e-8
    ):
    ...
    def __call__(
        ...
        # Compare rmsd_not_aligned before/after permutation
    ...
    def crop_label_dict_using_pred_dict(
        self,
        pred_dict: dict[str, torch.Tensor],
        label_full_dict: dict[str, torch.Tensor],
    ):
        self.process_input(pred_dict, label_full_dict, -1)
        permuted_indices = self.build_permuted_indice(self.unpermuted_match)

        return self.unpermuted_match, permuted_indices
...
def crop_label_dict_using_pred_dict(
    ...
    with torch.no_grad():
        (
            match,
            permuted_indices,
        ) = MultiChainPermutation().crop_label_dict_using_pred_dict(
            pred_dict={**input_feature_dict, "coordinate": pred_coord},
            label_full_dict=label_full_dict,
        )
    permuted_indices = permuted_indices.tolist()
    output_dict = {
        "coordinate": label_full_dict["coordinate"][permuted_indices, :],
        "coordinate_mask": label_full_dict["coordinate_mask"][permuted_indices],
    }
    # Permute extra label features, along the last dimension.
    output_dict.update(
        {
            k: label_full_dict[k][..., permuted_indices]
            for k in extra_label_keys
            if k in label_full_dict
        }
    )
