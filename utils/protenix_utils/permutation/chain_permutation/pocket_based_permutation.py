...
def permute_pred_to_optimize_pocket_aligned_rmsd(
    ...
    def _find_protein_ligand_chains_for_one_sample(
        ...
        for poc_asym_id, pocket_mask in candidate_pockets.items():
            ...
            if lig_rmsd < best_results.get("rmsd_not_aligned", torch.inf):
                best_results = {
                    "rmsd_not_aligned": lig_rmsd,
            ...
            if poc_asym_id == pocket_asym_id:
                ...
                unpermuted_results = {
                    "rmsd_not_aligned": unpermuted_lig_rmsd,
                    "aligned_pred_coord": aligned_pred_coord,
                }
        ...
        improved_rmsd = (unpermuted_results["rmsd_not_aligned"] - best_results["rmsd_not_aligned"]).item()
        if improved_rmsd >= 1e-12:
            ...
            )  # TODO: revert in this case, but this should not happen
        ...
        per_sample_log_dict["rmsd_not_aligned"] = best_results["rmsd_not_aligned"].item()
    ...
    # rmsd_not_aligned variance
    all_sample_rmsd = torch.tensor([x["rmsd_not_aligned"] for x in sample_log_dicts]).float()
    log_dict.update(
        {
