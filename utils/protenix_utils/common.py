...
def centre_random_augmentation(
    ...
    # ([..., N_sample, 3, 3], [..., N_sample, 3])
    batch_size_shape = x_input_coords.shape[:-3]
    rot_matrix_random = (
        _uniform_random_rotation(N_sample=N_augment)
        .to(device)
        .reshape(*batch_size_shape, N_sample, 3, 3)
    ).detach()
    trans_random = s_trans * torch.randn(size=(*batch_size_shape, N_sample, 3)).to(device) 
    x_augment_coords = (
...
def _uniform_random_rotation(N_sample: int = 1) -> torch.Tensor:
...
def batched_gather(
    ...
    remaining_dims[dim - no_batch_dims if dim >= 0 else dim] = inds # type: ignore
